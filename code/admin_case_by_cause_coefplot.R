...
CAUSE_LABELS <- c(
  ...
  labor_social = "Labor &\nSocial Security",
  administrative_act = "Generic\nAdmin Acts",
  economic_resource = "Economic &\nResource"
)
...
read_panels <- function(case_path, city_path) {
  ...
  cause_case_n <- cases[!is.na(cause_group), .(case_total = .N), by = cause_group]
  attr(panel, "cause_case_n") <- cause_case_n
  panel
}
...
plot_coefplot <- function(results_dt, baseline_dt, case_n_dt, outcome_label, file_path) {
  results_dt <- copy(results_dt)
  setorder(results_dt, -estimate)
  ordered_groups <- results_dt$cause_group
  baseline_dt <- baseline_dt[match(ordered_groups, cause_group)]
  case_n_dt <- case_n_dt[match(ordered_groups, cause_group)]
  ...
  fig_width <- max(8.4, 1.05 * nrow(results_dt))
  pdf(file = file_path, width = fig_width, height = 5.6, family = "serif")
  op <- par(
    bty = "l",
    las = 1,
    tcl = -0.25,
    mar = c(7.8, 5.2, 2.0, 1.0),
    cex.axis = 0.95,
    cex.lab = 1.1,
    xpd = FALSE
  )
  ...
  segments(x_left, 0, x_right, 0, col = "black", lwd = 1)
  ...
  par(xpd = NA)
  group_labels <- CAUSE_LABELS[results_dt$cause_group]
  for (i in seq_len(nrow(results_dt))) {
    mtext(group_labels[i], side = 1, line = 1.4, at = results_dt$x_pos[i],
          cex = 0.78, padj = 0.5)
  }
  ...
  baseline_labels <- vapply(seq_len(nrow(results_dt)), function(i) {
    ...
    cn <- case_n_dt[cause_group == cg]
    sprintf(
      "Pre %s (SD %s)\nN = %s",
      fmt_num(bl$pre_mean, 2), fmt_num(bl$pre_sd, 2), fmt_int(cn$case_total)
    )
  }, character(1))
  for (i in seq_len(nrow(results_dt))) {
    mtext(baseline_labels[i], side = 1, line = 4.0, at = results_dt$x_pos[i],
          cex = 0.62, padj = 0.5)
  }
  ...
  for (i in seq_len(nrow(results_dt))) {
    ...
    text(results_dt$x_pos[i], results_dt$ci_hi[i] + 0.025 * y_span,
         label, cex = 0.78, adj = c(0.5, 0))
  }
}

build_table <- function(results_dt, baseline_dt, case_n_dt, file_path) {
  results_dt <- copy(results_dt)
  setorder(results_dt, -estimate)
  ordered_groups <- results_dt$cause_group
  baseline_dt <- baseline_dt[match(ordered_groups, cause_group)]
  ...
  short_labels <- c(
    ...
    administrative_act = "Generic Acts",
    economic_resource = "Econ./Resource"
  )
  ...
  lines <- c(
    ...
    "\\addlinespace",
    paste("Cases in cause group &", paste(vapply(seq_len(nrow(results_dt)),
        function(i) fmt_int(case_n_dt$case_total[i]), character(1)),
      collapse = " & "), "\\\\"),
    ...
    "\\footnotesize",
    paste(
      "\\item \\textit{Notes:} Each column reports Treatment $\\times$ Post from a separate two-way fixed-effects regression on a (city $\\times$ year) panel restricted to one cause group.",
...
main <- function() {
  ...
  case_n_dt <- attr(panel, "cause_case_n")
  ...
  plot_coefplot(
    ...
    case_n_dt = case_n_dt,
    outcome_label = "ATE on Government Win Rate",
